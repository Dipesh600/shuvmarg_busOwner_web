...
import CrewIdentityLookup from "./CrewIdentityLookup";

interface Props {
  brands: OperatorBrand[]; initialRole: StaffRole; existing?: StaffMember | null;
  initialMode?: AddMode; initialBrandId?: string;
  onClose: () => void; onSaved: (message: string, warning: boolean, result: CrewAssignmentResult) => void;
}

type DriverStep = "details" | "documents";
type AddMode = "lookup" | "new";
...
export default function CrewAssignmentDialog({ brands, initialRole, existing, initialMode = "lookup", initialBrandId, onClose, onSaved }: Props) {
  ...
  const [brandId, setBrandId] = useState(existing?.brandId || initialBrandId || brands[0]?.id || "");
  ...
  const validDriverDetails = role !== "driver" || (Boolean(gender)
  ...
  const submit = async () => {
    ...
    if (role === "driver") {
      payload = { role, brandId, name: name.trim(), phone: phone.trim(),
        ...
        licenseType, licenseExpiry, ...(licenseDoc ? { licenseDoc } : {}) };
    } else payload = { role, brandId, name: name.trim(), phone: phone.trim() };
  ...
  const title = needsSecurityRefresh ? "Complete driver security check" : existing?.status === "INACTIVE"
      ? `Rehire ${role}` : `Add ${role}`;
  const lookupMode = !existing && addMode === "lookup";
    ...
    <div className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-3xl bg-neutral-50 shadow-2xl">
      <header className="sticky top-0 z-10 border-b border-neutral-200 bg-white px-6 py-5">
        <div className="flex items-start justify-between"><div><h2 id="crew-dialog-title" className="text-xl font-bold capitalize text-neutral-900">{title}</h2>
          <p className="mt-1 text-sm text-neutral-500">{lookupMode
            ? `Check an existing Shuvmarg ${role} account.`
            : `Create or connect this ${role}'s Partner app account.`}</p></div>
        ...
        {role === "driver" && !lookupMode && <div className="mt-4 grid grid-cols-2 gap-2" role="tablist" aria-label="Driver form steps">
      ...
      <div className="space-y-5 p-6">
        {!existing && <div className="grid grid-cols-2 gap-2 rounded-xl bg-neutral-100 p-1" role="tablist" aria-label={`${role} source`}>
          <button type="button" role="tab" aria-selected={addMode === "lookup"} onClick={() => { setAddMode("lookup"); setError(""); }} className={`rounded-lg px-3 py-2.5 text-sm font-bold ${addMode === "lookup" ? "bg-white text-[#7A1D1B] shadow-sm" : "text-neutral-500"}`}>Existing account</button>
          <button type="button" role="tab" aria-selected={addMode === "new"} onClick={() => { setAddMode("new"); setError(""); }} className={`rounded-lg px-3 py-2.5 text-sm font-bold ${addMode === "new" ? "bg-white text-[#7A1D1B] shadow-sm" : "text-neutral-500"}`}>New {role}</button>
        </div>}

        {lookupMode ? <CrewIdentityLookup role={role} brands={brands} initialBrandId={initialBrandId} onConnected={onSaved} /> : <>
        {(role !== "driver" || step === "details") && <>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {role === "driver" && <><label className="block text-xs font-bold text-neutral-700">Gender<select value={gender} onChange={event => setGender(event.target.value as typeof gender)} className={inputClass}><option value="">Select gender</option><option value="male">Male</option><option value="female">Female</option><option value="other">Other</option></select></label>
              <label className="block text-xs font-bold text-neutral-700">Experience (years)<input type="number" min={0} max={80} step={1} value={experienceYears} onChange={event => setExperienceYears(Number(event.target.value))} className={inputClass} /></label></>}
          </div>
        </>}
        ...
        {role === "driver" && step === "documents" && <div className="rounded-2xl border border-neutral-200 bg-white p-4">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            {(!existing || needsSecurityRefresh) && <label className="block text-xs font-bold text-neutral-700 sm:col-span-2">Licence document
      ...
      <footer className="sticky bottom-0 flex justify-end gap-3 border-t border-neutral-200 bg-white px-6 py-4"><button type="button" disabled={busy} onClick={onClose} className="rounded-xl border border-neutral-200 px-4 py-2.5 text-sm font-bold text-neutral-700">{lookupMode ? "Close" : "Cancel"}</button>
        {!lookupMode && <>
        {role === "driver" && step === "details" ? <button type="button" disabled={busy} onClick={() => { if (!validIdentity || !validDriverDetails) setError("Complete the required driver details first."); else { setError(""); setStep("documents"); } }} className="rounded-xl bg-[#7A1D1B] px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">Next: Documents</button>
          : <button type="button" disabled={busy} onClick={() => void submit()} className="flex min-w-32 items-center justify-center gap-2 rounded-xl bg-[#7A1D1B] px-5 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy && <LoaderCircle className="h-4 w-4 animate-spin" />}{needsSecurityRefresh ? "Run security checks" : existing?.status === "INACTIVE" ? "Rehire" : `Add ${role}`}</button>}</>}
      </footer>
    </div>
  </div>;
}
