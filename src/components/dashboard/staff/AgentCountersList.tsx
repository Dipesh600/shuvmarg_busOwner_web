...
const STATUS_LABELS: Record<AssignmentStatus, string> = {
  ACTIVE: "Agent access active", INVITED: "Invitation pending", SUSPENDED: "Access paused by you",
  ...
  return <section className="space-y-5">
    ...
    {loading && assignments.length === 0 ? <div className="flex justify-center rounded-3xl border border-neutral-200 bg-white p-16"><LoaderCircle className="h-7 w-7 animate-spin text-[#7A1D1B]" /></div>
      ...
      : <div className="grid gap-4 lg:grid-cols-2">{visible.map((row) => <article key={row.assignmentId} className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
        ...
        {row.status === "INVITED" && <p className="mt-4 rounded-xl border border-amber-100 bg-amber-50 p-3 text-xs font-medium text-amber-800">The settings below start only after the agent accepts this invitation.</p>}
        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-neutral-500">Brand</dt><dd className="font-semibold text-neutral-800">{row.brand.name || "Unknown"}</dd></div><div><dt className="text-xs text-neutral-500">Tickets sold</dt><dd className="font-semibold text-neutral-800">{row.salesCount ?? 0}</dd></div><div><dt className="text-xs text-neutral-500">{row.status === "INVITED" ? "Planned trip access" : "Trip access"}</dt><dd className="font-semibold text-neutral-800">{ACCESS_LABELS[row.access.accessScope]}</dd></div><div><dt className="text-xs text-neutral-500">{row.status === "INVITED" ? "Planned cash booking access" : "Cash bookings"}</dt><dd className="font-semibold text-neutral-800">{row.permissions.canSellCash ? "Allowed" : "Not allowed"}</dd></div><div className="col-span-2"><dt className="text-xs text-neutral-500">{row.status === "INVITED" ? "Planned commission" : "Commission"}</dt><dd className="font-semibold text-neutral-800">{commissionLabel(row)}</dd></div></dl>
