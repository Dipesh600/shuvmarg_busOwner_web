...
export interface TripsPageHeaderProps {
  totalTrips?: number;
  onRefresh?: () => void;
      ...
      <div className="relative z-10 flex flex-col justify-between p-5 sm:p-8 md:p-9 min-h-[175px] sm:min-h-[220px] md:min-h-[230px]">
        <div className="max-w-md sm:max-w-lg space-y-1">
          ...
          <p className="text-xs sm:text-sm text-[#554E48]">
            Manage active departures, inspect capacity, and preview scoped service changes across your fleet.
          </p>
        </div>

        {/* Optional Action Controls / Children */}
        {children && (
          <div className="mt-4 sm:mt-0 sm:absolute sm:bottom-7 sm:right-7 md:bottom-8 md:right-9 flex flex-wrap items-center gap-2.5">
            {children}
          </div>
        )}
      </div>
